...
int main() {
    ...
    do {
        ...
        cout << "3. Menu Pengurutan\n";
        cout << "4. Menu Pencarian\n";
        cout << "5. Hapus Obat\n";
        ...
        if (pilihan == 1) {
			system("cls");
            Obat o;
            cout << "ID Obat: "; cin >> o.id;
            cout << "Nama Obat: "; cin.ignore(); cin.getline(o.nama, 30);
            cout << "Stok: "; cin >> o.stok;
            ...
            tampilkanData();
            tekanEnter();
        } else if (pilihan == 3) { // SUB-MENU SORTING
            if (listKosong()) { cout << "Data kosong!\n"; continue; }
            int n = hitungNode();
            Obat *tempArr = new Obat[n];
            typeptr bantu = awal;
            for (int i = 0; i < n; i++) { tempArr[i] = bantu->info; bantu = bantu->kanan; }
            
            int pSort;
            system("cls");
            cout << "\n--- SUB-MENU PENGURUTAN ---\n";
            cout << "1. Berdasarkan ID (Ascending)\n";
            cout << "2. Harga (Ascending)\n";
            cout << "3. Harga (Descending)\n";
            cout << "Pilih: ";
            cin >> pSort;
            
            quickSort(tempArr, 0, n - 1, pSort);
            buatListBaru();
            for (int i = 0; i < n; i++) tambahObat(tempArr[i]);
            delete[] tempArr;
            cout << "Berhasil diurutkan.\n";
            tampilkanData();
            tekanEnter();
        } else if (pilihan == 4) { // SUB-MENU SEARCHING
            if (listKosong()) { cout << "Data kosong!\n"; continue; }
            int n = hitungNode();
            Obat *tempArr = new Obat[n];
            typeptr bantu = awal;
            ...
            if (pCari == 1) {
                int idK; cout << "Masukkan ID: "; cin >> idK;
                int h = cariBerdasarkanID(tempArr, n, idK);
                if (h != -1) cout << "Ditemukan: " << tempArr[h].nama << " | Harga: Rp" << tempArr[h].harga << endl;
                else cout << "Tidak ditemukan.\n";
            } else if (pCari == 2) {
                char namaK[30]; cout << "Masukkan Nama: "; cin.ignore(); cin.getline(namaK, 30);
                int h = cariBerdasarkanNama(tempArr, n, namaK);
                if (h != -1) cout << "Ditemukan ID: " << tempArr[h].id << " | Harga: Rp" << tempArr[h].harga << endl;
                else cout << "Tidak ditemukan.\n";
            }
            delete[] tempArr;
            tekanEnter();
            ...
            system("cls");
            tampilkanData();
            int idH; cout << "\nMasukkan ID yang akan dihapus: "; cin >> idH;
            hapusObat(idH);
            tekanEnter();
        }
    } while (pilihan != 6);
    ...
    system("cls");
    cout << "==========================================\n";
    cout << "  DATA TERSIMPAN. PROGRAM SELESAI.       \n";
    cout << "==========================================\n";
    return 0;
}
